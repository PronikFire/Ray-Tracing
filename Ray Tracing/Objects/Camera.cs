...
public class Camera(float fov, Vector2u size, Scene scene) : Object
{
    ...
    private async Task Raycast(uint x, uint y)
    {
        ...
        Ray ray = new(Position, Vector3.Transform(Vector3.UnitZ, rayRotation * Rotation));
        ...
        foreach (Object obj in Scene.Objects)
        {
            ...
            if (result != null && Vector3.Distance(Position, iResult.Point) > Vector3.Distance(Position, result.Value.Point))
                continue;
        ...
        foreach (Light light in Scene.Lights)
        {
            ...
            Vector3 localLightPos = light.Position - result.Value.Point;
        ...
        byte dCamera = (byte)(Vector3.Dot(Vector3.Normalize(result.Value.Normal), Vector3.Normalize(Position - result.Value.Object.Position)) * 127 + 127);
